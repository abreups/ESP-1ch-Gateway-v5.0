...
void printHexDigit(uint8_t digit) {
    if(digit < 0x10) Serial.print('0');
    Serial.print(digit,HEX);
} // end of printHexDigit()
...
static void printTime() {
    ...
    Serial.print(F(" ")); printDigits(hour());
    Serial.print(F(":")); printDigits(minute());
    Serial.print(F(":")); printDigits(second());
    Serial.print(F(" "));
    return;
} // end of printTime()
...
// Each line contains an KEY value pair describing the gateway configuration
// ----------------------------------------------------------------------------
int WlanReadWpa() {
...
int WlanWriteWpa( char* ssid, char *pass) {
	printTime(); Serial.print(F("WlanWriteWpa:: ssid=")); Serial.print(ssid);
...
int WlanConnect(int maxTry) {
    ...
    while ( (WiFi.status() != WL_CONNECTED) && ( i < maxTry ) ) {
        ...
        for (int j=wpa_index; j< (sizeof(wpa)/sizeof(wpa[0])) ; j++) {
            ...
            Serial.print(i); Serial.print(':'); Serial.print(j); Serial.print(F(". WiFi connect to: ")); Serial.println(ssid);
...
#if DUSB>=1
    if (i>0) Serial.println(F("WLAN reconnected"));
#endif
  
    // Still not connected?
    if (WiFi.status() != WL_CONNECTED) {
#if WIFIMANAGER==1
        printTime(); Serial.println(F("Starting Access Point Mode"));
        Serial.print(F("Connect Wifi to accesspoint: ")); Serial.println(AP_NAME);
        Serial.println(F(" and connect to IP: 192.168.4.1"));
...
int readUdp(int packetSize) {
...
#if DUSB>=1
    printTime(); Serial.println(F("ESP-sc-gway::readUsb:: UDP packet reading error - fewer chars"));
		return(-1);
#endif
	}
	...
	if (remotePortNo == 123) { // This is an NTP message arriving (port 123 is ntp)
#if DUSB>=2
    printTime(); Serial.println(F("ESP-sc-gway::readUdp:: NTP msg rcvd from LoRa server"));
#endif
		gwayConfig.ntpErr++;
		gwayConfig.ntpErrTime = millis();
		return(0);
	} else { // If it is not NTP it must be a LoRa message for gateway or node
		uint8_t *data = (uint8_t *) ((uint8_t *)buff_down + 4);
		protocol = buff_down[0];
		token = buff_down[2]*256 + buff_down[1];
		ident = buff_down[3];
#if DUSB>=2
    printTime(); Serial.print("ESP-sc-gway::readUdp::protocol = 0x"); Serial.println(protocol, HEX);
    printTime(); Serial.print("ESP-sc-gway::readUdp::token = "); Serial.println(token);
    printTime(); Serial.print("ESP-sc-gway::readUdp::ident = 0x"); Serial.println(ident, HEX);
#endif
        ...
        //-----------------------------------------------------------------------
#if DUSB>=2
            printTime(); Serial.print("ESP-sc-gway::readUdp::ident = PKT_PUSH_DATA = "); Serial.println(ident);
      			Serial.print(F("    size = ")); Serial.println(packetSize);
      			Serial.print(F("    From = ")); Serial.println(remoteIpNo);
      			Serial.print(F("    port = ")); Serial.println(remotePortNo);
      			Serial.print(F("    data = "));
        ...
        //-----------------------------------------------------------------------
#if DUSB>=2
            printTime(); Serial.print("ESP-sc-gway::readUdp::ident = PKT_PUSH_ACK = "); Serial.println(ident);
      			Serial.print(F("    size = ")); Serial.println(packetSize);
      			Serial.print(F("    From = ")); Serial.println(remoteIpNo);
      			Serial.print(F("    port = ")); Serial.println(remotePortNo);
        ...
        //-----------------------------------------------------------------------
#if DUSB>=2
            printTime(); Serial.print("ESP-sc-gway::readUdp::ident = PKT_PULL_DATA = "); Serial.println(ident);
#endif
            break;
        ...
        //-----------------------------------------------------------------------
#if DUSB>=1
            printTime(); Serial.print("ESP-sc-gway::readUdp::ident = PKT_PULL_RESP = "); Serial.println(ident);
#endif
            ...
            buff[12]=0;
#if DUSB>=2
            printTime(); Serial.println(F("ESP-sc-gway::readUdp:: TX buff filled"));
#endif
...
#endif
#if DUSB>=1
                printTime(); Serial.println("ESP-sc-gway::readUdp::PKT_PULL_ACK Error UDP write");
#endif
            } else {
#if DUSB>=2
                printTime(); Serial.print(F("ESP-sc-gway::readUdp::PKT_TX_ACK tmst=")); Serial.println(micros());
#endif
            }
            ...
            yield();
#if DUSB>=2
            printTime(); Serial.print("ESP-sc-gway::readUdp::PKT_PULL_RESP size = "); Serial.print(packetSize);
        ...
        //-----------------------------------------------------------------------
#if DUSB>=1
            printTime(); Serial.print("ESP-sc-gway::readUdp::ident = PKT_PULL_ACK = "); Serial.println(ident);
...
#if DUSB>=2
    printTime();
		Serial.print(F("ESP-sc-gway::readUdp returning packetSize = ")); 
		Serial.println(packetSize);
#endif
...
// Create a packet with Udp.beginPacket(), then fill in the data with 
//   Udp.write() and transmits the data with Udp.endPacket().
...
int sendUdp(IPAddress server, int port, uint8_t *msg, int length) {
	...
	if (WlanConnect(3) < 0) {
#if DUSB>=1
    printTime(); Serial.println("ESP-sc-gway::sendUdp:: ERROR connecting to WiFi");
		Serial.flush();
#endif
		Udp.flush();
		yield();
	...
	yield();
//#if DUSB>=1
//    printTime(); Serial.println(F("ESP-sc-gway::sendUdp::WiFi connected"));
//#endif	
...
#if DUSB>=1
        printTime(); Serial.println(F("ESP-sc-gway::sendUdp:: Error Udp.beginPacket"));
#endif
		    return(0); // return error
	  } else {
#if DUSB>=2
        printTime(); Serial.print("ESP-sc-gway::sendUdp::server = "); Serial.print(server);Serial.print(", port = "); Serial.println(port);
#endif
	  }
	  yield();
#ifdef ESP32BUILD
...
#endif
#if DUSB>=1
        printTime(); Serial.println(F("ESP-sc-gway::sendUdp:: Error write"));
#endif
		    Udp.endPacket();  // Close UDP
		    return(0);        // return error
	  } else {
#if DUSB>=2
        printTime(); Serial.print("ESP-sc-gway::sendUdp::message = ");
...
#if DUSB>=2
        printTime(); Serial.println(F("ESP-sc-gway::sendUdp::UDP message sent"));
#endif
  	}
...
#if DUSB>=1
  printTime(); Serial.print(F("ESP-sc-gway::UDPconnect::Local UDP port="));
...
void pullData() {
    ...
    //send the update
	  ...
	  uint8_t *pullPtr;
	  pullPtr = pullDataReq,
#ifdef _TTNSERVER // TTN server or your server if you replaced TTN by yours
#if DUSB>=2
    printTime(); Serial.println("ESP-sc-gway::pullData::calling sendUDP() to send packet to server");
#endif
	...
	yield();
	  ...
	  if (pullPtr != pullDataReq) {
		    printTime(); Serial.println(F("ESP-sc-gway::pullData:: ERROR: pullPtr != pullDatReq"));
		    Serial.flush();
	  }
#endif

#ifdef _THINGSERVER // additional server (private server)
#if DUSB>=2
    printTime(); Serial.println("ESP-sc-gway::pullData::calling sendUDP to send packet to server");
#endif
...
#if DUSB>=2
	  yield();
    printTime(); Serial.print("ESP-sc-gway::pullData::PKT_PULL_DATA request, len = "); Serial.print(pullIndex); Serial.print(", data = ");
...
// ----------------------------------------------------------------------------
// Send UP periodic status message to server even when we do not receive any data. 
// Parameters: <none>
// ----------------------------------------------------------------------------
void sendstat() {
	...
	if (stat_index > STATUS_SIZE) {
		printTime(); Serial.println("ESP-sc-gway::sendstat::ERROR buffer too big");
		return;
	}
...
#ifdef _TTNSERVER
#if DUSB>=2
    printTime(); Serial.println("ESP-sc-gway::sendstat() will call ESP-sc-gway::sendUdp()");
#endif
...
void setup() {
  ...
  display.init();
	...
	Serial.print("MAC: "); Serial.print(MAC_char);
	Serial.print(F(", len=")); Serial.println(strlen(MAC_char));
  ...
  Serial.print(", Listening at SF"); Serial.print(sf); Serial.print(" on "); Serial.print((double)freq/1000000); Serial.println(" Mhz.");
	...
	while (timeStatus() == timeNotSet) {
		  Serial.println(F("ESP-sc-gway::setup:: Time not set (yet)"));
		  delay(500);
		  // time_t newTime;
		  time_t newTime = (time_t)getNtpTime();
		  if (newTime != 0) setTime(newTime);
	}
	Serial.print("NTP Time is: "); printTime(); Serial.println();

	writeGwayCfg(CONFIGFILE );
	Serial.println(F("Gateway configuration saved to SPIFFS"));
#endif

#if A_SERVER==1	
	// Setup the webserver
  printTime(); Serial.println("ESP-sc-gway::setup::calling setupWWW()");
	setupWWW();
#endif
...
#if DUSB>=2
	printTime(); Serial.println("ESP-sc-gway::setup::calling initLoraModem()");
#endif
	initLoraModem();
	...
	if (_cad) {
		...
		_state = S_RX;
#if DUSB>=2
  printTime(); Serial.println("ESP-sc-gway::setup::calling rxLoraModem()");
#endif
		rxLoraModem();
	}
...
void loop () {
    ...
    if (_event != 0x00) {
#if DUSB>=2
        printTime(); Serial.print("ESP-sc-gway::loop::_event = 0x"); Serial.print(_event, HEX); Serial.println(", calling stateMachine()");
#endif
        stateMachine();     // start the state machine
#if DUSB>=2
        printTime(); Serial.print("ESP-sc-gway::loop::returning from call to stateMachine(). Setting _event = 0");
#endif
        _event = 0;         // reset value
        return;							// Restart loop
    }
	
    // After a quiet period, make sure we reinit the modem. // WHY????
    // XXX Still have to measure quiet period in stat[0];
    // For the moment we use msgTime
    if ( (((nowTime - statr[0].tmst) / 1000000) > _MSG_INTERVAL ) && (msgTime < statr[0].tmst) ) {
#if DUSB>=1
        printTime(); Serial.println("ESP-sc-gway::loop::reinitiating lora modem after a quiet period");
#endif
        initLoraModem();
        if (_cad) {
                _state = S_SCAN;
                ...
                _state = S_RX;
#if DUSB>=2
        printTime(); Serial.println("ESP-sc-gway::loop::calling rxLoraModem in quiet period routine");
#endif
                rxLoraModem();
        }
#if DUSB>=2
        printTime(); Serial.println("ESP-sc-gway::loop::reseting interrupts in quiet period routine");
#endif
    ...
    yield();
#if DUSB>=2
    printTime(); Serial.println("ESP-sc-gway::loop::Perform OTA update");
#endif
    ArduinoOTA.handle();
#endif
    ...
    printTime(); Serial.println("ESP-sc-gway::loop::handle WiFi");
#endif
    server.handleClient();	
#endif
    ...
    if (WlanConnect(1) < 0) {
#if DUSB>=1
        printTime(); Serial.println("ESP-sc-gway::loop::WlanConnect failed");
#endif
        yield();
...
#if DUSB>=5
        printTime(); Serial.println(F("ESP-sc-gway::loop::WlanConnect success"));
#endif
...
#if DUSB>=2
            printTime(); Serial.println(F("ESP-sc-gway::loop::reading Udp packet"));
#endif
...
#if DUSB>=1
                printTime(); Serial.println(F("ESP-sc-gway::readUDP::error reading UDP packet"));
#endif
                break;
            } else { // Now we know we succesfull received message from host
#if DUSB>=2
                printTime(); Serial.println(F("ESP-sc-gway::loop::success reading UDP packet"));
                printTime(); Serial.println(F("ESP-sc-gway::loop::setting _event=0"));
#endif
	...
	if ((_hop) && (((long)(nowTime - hopTime)) > 7500)) {
...
#if DUSB>=1
      		printTime(); Serial.println(F("ESP-sc-gway::loop::calling hop()"));
#endif
			hop(); 
		}
		...
		// If we keep staying in another state, reset
			  ...
			  _state= S_RX;
#if DUSB>=2
        printTime(); Serial.println("ESP-sc-gway::loop::calling rxLoraModem()");
#endif
			  rxLoraModem();
#if DUSB>=2
        printTime(); Serial.println("ESP-sc-gway::loop::calling hop()");
#endif
			  hop();
			  if (_cad) { 
				    _state= S_SCAN; 
#if DUSB>=2
            printTime(); Serial.println("ESP-sc-gway::loop::calling cadScanner()");
#endif
				    cadScanner(); 
			  }
		}
#if DUSB>=2
		printTime(); Serial.print(F("ESP-sc-gway::loop::state=")); Serial.println(_state);
#endif
...
#if DUSB>=2
      	printTime(); Serial.print("ESP-sc-gway::loop::calling sendstat()");
#endif
    ...
    if ((nowSeconds - pulltime) >= _PULL_INTERVAL) {	// Wake up every xx seconds
#if DUSB>=2
    		printTime(); Serial.println("ESP-sc-gway::loop::Send PULL_DATA message to server");
        Serial.flush();
#endif
        pullData();										// Send PULL_DATA message to server
    		...
    		writeRegister(REG_IRQ_FLAGS, 0xFF);				// Reset all interrupt flags
#if DUSB>=1
  		  printTime(); Serial.println("ESP-sc-gway::loop::end of sending PULL_DATA message to server");
#endif		
		    pulltime = nowSeconds;
    }
